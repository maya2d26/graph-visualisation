...
def final_pos_to_csv(g, folder = POS_FOLDER):
    ...
    id = g.graph['id']
    type = g.graph['type']
    # if the type is star or grid, the positions weren't generated by an algorithm
    if type != 'random':
        algorithm = type
    else:
...
def visualise(g, figsize = (8,8), file = None):
    ...
    # illustrate graph 
    nx.draw(g, pos=pos,node_size = 75, width=0.03,  ax = plt.gca())
    if file:
        plt.savefig(file)
    plt.show()
...
def calculate_num_inersections(G):
    ...
    for e1, e2 in combinations(G.edges, 2):
        ...
        pos_e1 = (p1, p2)
        pos_e2 = (q1, q2)
        if check_intersect(pos_e1, pos_e2):
            num_intersections = num_intersections + 1
    
    return num_intersections - G.number_of_nodes()

def calculate_symmetry(G):
    nodes = [G.nodes[n]['pos'] for n in G.nodes]
    nodes = np.array(nodes)
    center = nodes.mean(axis=0)
    reflected_nodes = 2 * center - nodes
    displacements = nodes - reflected_nodes
    rms_displacement = np.sqrt(np.sum(np.sum(displacements**2, axis=1)))
    average_displacement = rms_displacement / G.number_of_nodes()
    distance_from_center = np.linalg.norm(nodes - center, axis=1)
    average_distance = np.mean(distance_from_center)
    symmetry_score = 1 - average_displacement / average_distance
    return symmetry_score

def calculate_horizontal_symmetry(G):
    nodes = [G.nodes[n]['pos'] for n in G.nodes]
    nodes = np.array(nodes)
    center = nodes.mean(axis=0)
    reflected_nodes = 2 * center - nodes
